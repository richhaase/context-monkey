...
export function registerApply(program: Command): void {
  program
    .command("apply")
    .description("Apply a portable IR bundle to a target harness")
    .argument("<bundle>", "path to cm-context.json bundle")
    ...
    .action(
      async (
        bundlePath: string,
        targetId: string,
        opts: { path: string; yes?: boolean; categories?: string },
      ) => {
        ...
        try {
          const raw = await Bun.file(resolve(bundlePath)).text();
          bundle = JSON.parse(raw);
        } catch {
          console.error(chalk.red(`  Could not read bundle: ${bundlePath}`));
          process.exit(1);
        }

        if (bundle.version !== 1) {
          console.error(chalk.red(`  Unsupported bundle version: ${bundle.version}`));
          process.exit(1);
        }
        ...
        console.log(chalk.dim(`  Bundle: ${bundlePath} (${bundle.items.length} items)`));
        ...
        // Convert bundle items to ContextEntry[]
        const entries: ContextEntry[] = bundle.items
          ...
          .map((item) => ({
            ...
            sourcePath: `bundle:${item.sourceHarness}/${item.name}`,
            scope: item.scope,
            raw: "", // bundles don't carry raw content
          }));
