...
async function install(options = {}) {
  const { global = false, _skipExistingCheck = false } = options;
  ...
  // Check for existing installation (unless skipped by upgrade command)
  const existingPath = path.join(installPath, 'commands', 'monkey');
  if (!_skipExistingCheck && exists(existingPath)) {
    console.log(`Context Monkey is already installed in ${displayPath}.`);
    console.log('Use `npx context-monkey upgrade` to update to the latest version.');
    return;
  }
